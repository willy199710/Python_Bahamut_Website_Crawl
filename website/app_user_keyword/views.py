...
def load_df_data():
    ...
    global df
    df = pd.read_csv(
        'app_user_keyword/dataset/bahamut_preprocessed_data.csv', sep='|')
...
def filter_dataFrame(key, cond, cate, weeks):
    ...
    # proceed filtering
    if (cate == "全部") & (cond == 'and'):
        query_df = df[(df.date >= start_date) & (df.date <= end_date) & df.tokens.apply(
            lambda row: all((qk in row) for qk in key))]
    elif (cate == "全部") & (cond == 'or'):
        queryKey = '|'.join(key)
        query_df = df[(df['date'] >= start_date) & (
            df['date'] <= end_date) & df.tokens.str.contains(queryKey)]
    elif (cond == 'and'):
        query_df = df[(df.category == cate) & (df.date >= start_date) & (df.date <= end_date) & df.tokens.apply(
            lambda row: all((qk in row) for qk in key))]
    elif (cond == 'or'):
        queryKey = '|'.join(key)
        query_df = df[(df.category == cate) & (df['date'] >= start_date) & (df['date'] <= end_date) & df[
            'tokens'].str.contains(queryKey)]
...
news_categories = ['全部', '電腦應用綜合討論', '電視遊樂器綜合討論', '智慧型手機']
